...
5. Follow the on-screen steps — the wizard creates the API token, generates the `helm` or `kubectl` commands, and guides you through applying them to the cluster
6. Verify the cluster appears in **Kubernetes** with nodes and workloads reporting
...
7. Before applying the wizard-generated config, remove the AppArmor volume from the `node-config-collector` DaemonSet definition. Amazon Linux 2 / AL2023 nodes do not have `/sys/kernel/security/apparmor` (they use SELinux, not AppArmor), so the pod will stay in `ContainerCreating` if you apply the config as-is.

   In the generated YAML, find the `node-config-collector` DaemonSet and delete both the volume and its mount:

   In `spec.template.spec.volumes`, remove:
   ```yaml
   - hostPath:
   ...
   In `spec.template.spec.containers[].volumeMounts`, remove the entry with `name: node-path-5`.

   Then apply the edited config normally.
