...
[EasyTrade](https://github.com/Dynatrace/easytrade/tree/main) is a demo application of a fictitious stock broking application to allow users to buy and sell stocks. The project consists of many small services that connect to each other and has built-in problems that can be toggled on/off for demoing Dynatrace monitoring. This guide deploys EasyTrade using Docker Compose on a VM.
...
**These other steps will require to be SSH'd into the VM, so do that now.**
...
# docker compose list project, should return empty list
